...
export const Usage = ({ usage }) => {
	...
	return (
		<UsageExcerpt
			element="article"
			aria-labelledby={`${kebabCase(usage.name)}-name ${kebabCase(usage.name)}-description`}
		>
				...
				<HeaderContentWrap>
					{image && (
						<BioFigureWrap>
							...
							>
								<BioImage fluid={image.childImageSharp.fluid} alt="Jonathan Harrell" />
								{[1, 2].map(index => (
						...
						<ProjectsWrap animate="mounted" variants={variants}>
							{involvement.projects.map((project, index) => (
								<ProjectWrap
									...
									initial={shouldAnimate() ? { opacity: 0, y: 50 } : false}
								>
									<Project element="article" aria-labelledby={`${kebabCase(project.name)}-name`}>
										<ProjectLink href={project.link} target="_blank" rel="noopener noreferrer">
											<span className="sr-only">Learn more</span>
										</ProjectLink>
										<div>
											<ProjectTitle>
												<Heading level={3} id={`${kebabCase(project.name)}-name`}>
													{project.name}
												</Heading>
											</ProjectTitle>
